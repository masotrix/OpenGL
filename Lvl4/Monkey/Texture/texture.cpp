...
GLuint loadBMP_custom(const char * imagepath){
	...
	if (!file){
        printf("%s could not be opened. In right directory ?\n",
                imagepath);
        getchar();
        return 0;
    }
...
GLuint loadDDS(const char * imagepath){
	...
	if (fp == NULL){
		printf("%s could not be opened. In the right directory?\n",
                imagepath); getchar(); 
		return 0;
	}
	...
	unsigned int blockSize = (format == 
            GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) ? 8 : 16, offset = 0;
	...
	for (unsigned int level = 0;
		...
		glCompressedTexImage2D(GL_TEXTURE_2D, level,
                format, width, height, 0, size, buffer + offset); 
