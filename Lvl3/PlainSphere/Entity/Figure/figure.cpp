...
void Figure::display(mat4& ViewPerspective){
...
void Figure::translate(glm::vec3 movement){
...
void Figure::fillColors(void){
    ...
    vector<vec3>* data = _colors->getData();
    for (int i=0; i<_vert; i++)
        for (int j=0; j<_dim; j++)
        (*data)[i][j] = ((GLfloat) rand() / (RAND_MAX));
...
template <class T>
vector<T>* Attribute<T>::getData(void){
